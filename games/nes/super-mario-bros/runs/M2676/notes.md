> **Imported**
> This run was originally published at https://tasvideos.org/2676M and entered this archive as a voluntary
...
Greetings, everyone. This is Mars608 and me ([user:HappyLee]) presenting you a brand new SMB warpless walkathon, which is a 650 frame (10.82 seconds) improvement over [1895M|Mars608's previous movie].

[http://www.youtube.com/watch?v=TzNhbg7oyGQ|Watch comparison video on YouTube]%%%
...
;1-3: 21 frames were saved for better optimization at the mushroom and the last Koopa. Having touched the flagpole 5 frames earlier, just enough to break a framerule.
...
;3-1: Just like the last movie, vine glitch was used and a special ending using the flagpole glitch on the right side of the screen. Mars608 did an extraordinary work on entertainment, including getting "8000" score twice, and the last shell kick near the flagpole was epic.
;3-2: 21 frames were saved for reasons known while making the first warpless walkathon TAS.
;3-3: Just like the previous movie, Mario went through the ground with the help of a balanced lift to search for a special ending. The movement I first found in the warpless run can actually save lots of time in warpless walkathon.
...
;4-3: 378 frames were saved for a better strategy of kicking the shell, which saved a time waiting for the Koopa's recovering. More time would be saved if it weren't for the fireworks that had to be avoided, so I did an extra walljump at the end to entertain.
;5-1: The first level in this movie using a Bullet Bill to skip the castle ending. The same old glitch was performed 4 times later in 5-2, 7-1, 8-2 and 8-3, and by coincidence, all 5 Bullet Bills used in this movie are the same height.
;5-2: 21 frames were lost due to lack of luck (see HappyLee's comments for more introductions). Also it's the first SMB TAS submitted here that has ever used a star, for fun.
;5-4: 21 frames were saved, mostly because Mars608 had to slow down to manipulate the Bowser during his last movie, but not this time.
;6-2: An obscure wall bump was used at the right side of the pipe below the vine to make the vine translate Mario further to the right edge of the screen, but still not enough to preform the marginal flagpole glitch, so Mario performed an extra walljump on a pipe before the vine. Although it's slightly faster to bump into another pipe or the block on the bottom of the flagpole, it's not entertaining enough and won't save time. Also I tried to grab the star as a wall bump but it would waste 21 frames, so it turned out that 5-2 was the only possible level to get a star without time loss.
...
;7-4: Mario went through the brick an extra time to manipulate screen X position and to make the castle maze more interesting, with no time loss (though slightly slower).
;8-4: 1 frame was saved in the second stage, at the walljump on the upper pipe, to our surprise. A moonwalk was performed later, just like before.

! Screenshots
[http://i292.photobucket.com/albums/mm22/HappyLee12/WarplessWalkathon4-1_zps9b936f06.png] [http://i292.photobucket.com/albums/mm22/HappyLee12/WarplessWalkathon6-4-2_zps5c17a6e0.png]
